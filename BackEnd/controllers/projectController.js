...
const ProjectHandle = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        const response = await axios.put(`/api/projects/${formData._id}`, formData);
  ...
  const handleDelete = async (_id) => {
    try {
      ...
      setProjects(prev => prev.filter(item => item._id !== _id));
    } catch (error) {
  ...
  return (
    <div className="min-h-screen p-8">
      <div className="relative z-10">
          ...
          <div className="mt-8 flex gap-4 items-center">
            <div className="flex bg-white/10 rounded-lg p-1">
              <button
                ...
                className={`px-4 py-2 rounded-md transition-colors ${viewMode === 'grid' ? 'bg-white/10' : ''}`}
              >
                <i className="ri-grid-fill"></i>
              </button>
              <button
                onClick={() => setViewMode('list')}
                className={`px-4 py-2 rounded-md transition-colors ${viewMode === 'list' ? 'bg-white/10' : ''}`}
              >
        ...
        <div
          ref={containerRef}
          className={`max-w-7xl mx-auto ${viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-3 gap-6' : 'space-y-4'}`}
        >
          {projects.map((project, index) => (
            <div
              key={project._id}
              ref={el => projectRefs.current[index] = el}
              className={`bg-neutral-900/50 backdrop-blur-sm rounded-lg overflow-hidden border border-white/10 hover:border-white/20 transition-all ${viewMode === 'grid' ? '' : 'flex items-center'}`}
            >
              ...
              <div className="p-4">
                <div className="flex items-start justify-between">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs ${project.status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                    {project.status}
                  </span>
                </div>
                <div className="mt-4 flex gap-2">
                  <button
                    onClick={() => {
                      handleEdit(project); // assumes useFormState supports _id
                      setIsModalOpen(true);
                    }}
