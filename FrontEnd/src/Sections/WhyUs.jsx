...
const WhyUs = () => {
  ...
  const reasons = [
    ...
    },
    {
      ...
      description: "We're available when you need us—WhatsApp, email—whatever works best for you.",
  ...
  return (
    <section ref={sectionRef} className="relative">
      ...
      <div className={`reasons-wrapper relative ${isMobile ? '' : 'h-screen'}`}>
        {/* Title - Made sticky for mobile */}
        <div className="sticky top-0 pt-16 pb-8  z-30">
          <h2 className="whyus-title text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-bold text-white text-center px-4">
            Why Choose Us
          </h2>
        </div>

        {/* Mobile layout */}
        <div className="md:hidden flex flex-col gap-6 px-4 pb-16">
          {reasons.map((reason, index) => (
            <div 
              key={index} 
              className="reason-card bg-gradient-to-br from-white/10 to-white/5 rounded-xl border border-white/10 p-6"
            >
              <div className="flex flex-col min-h-[200px]">
                <div className="text-white mb-4">{reason.icon}</div>
                <h3 className="text-xl font-bold text-white mb-3">{reason.title}</h3>
                <p className="text-sm text-[#EAE4D4] mb-4">{reason.description}</p>
